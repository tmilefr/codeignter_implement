...
?>
	...
	foreach($ctrls AS $key => $ctrl){
		...
		foreach($ctrl->actions AS $action){ 
			$allow = false;
			if (isset($acl_rc[$ctrl->id.'_'.$action->id]) && $acl_rc[$ctrl->id.'_'.$action->id] == 1){
				$allow = true;
			}
			...
			<div class="custom-control custom-switch form-check-inline">
				<input type="checkbox" <?php echo (($allow ) ? 'checked="checked"':'');?> class="custom-control-input" name="rules[]" id="customSwitch<?php echo $ctrl->id.'_'.$action->id;?>" value="<?php echo $ctrl->id.'_'.$action->id;?>">
